...
export class ProductListService {
  ...
  constructor(private store: Store<ShoppingState>) {
    this.productList = this.store.select(
      state => new ProductList(state.productList)
    );
  }

  public getPageNumbers(): Observable<number[]> {
    return this.productList.switchMap(productList =>
      Observable.range(1, productList.options.totalPages).toArray()
    );
  }

  public getCurrentPage(): Observable<number> {
    return this.productList.map(productList => productList.options.currentPage);
  }
  ...
  public getProductCategories(): Observable<ProductCategory[]> {
    return this.productList.map(productList => productList.productCategories);
  }
    ...
    return this.productList.map(productList => {
      return productList.productCategories.map(category => {
        return {
          ...
          name: category.name,
          selected:
            productList.options.selectedProductCategories.indexOf(
